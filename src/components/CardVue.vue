<template>
       <div class="flex flex-col border rounded-3xl p-8 relative">
            <img class=" w-8 absolute top-8 left-8" src="/icons/like_1.svg" alt="like">
            <div class="flex justify-center">
                <img class="flex w-32" src="/snickers/snickers_1.jpg" alt="snickers">
            </div>
            <p class="mt-4 flex justify-center">Мужские Кроссовки Nike Blazer Mid Suede</p>
            <div class="flex justify-between mt-4">
                <div class="flex flex-col">
                    <span class="text-gray-400">Цена:</span>
                    <span class="font-bold">12 999 руб.</span>
                </div>
                <div class="flex items-center">
                    <button><img src="/icons/plus_1.svg" alt="plus"></button>
                </div>
            </div>
        </div>
</template>